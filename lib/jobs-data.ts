...
type DbJobRow = {
  ...
  // Never put on the client-facing Job object below -- only used here,
  // server-side, to compute the public "unclaimed" boolean (a seeded listing
  // has no employer yet). claim_token itself is deliberately NEVER selected
  // by this query: it's a bearer secret, and JOB_COLUMNS feeds pages whose
  // data gets serialized straight into the page's HTML/RSC payload --
  // selecting it here would leak the exact credential needed to claim
...
function fromDbRow(row: DbJobRow): Job {
  return {
    ...
    unclaimed: row.employer_id === null,
  };
}
