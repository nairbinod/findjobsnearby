      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {visibleJobs.map((job, index) => (
          <Link href={jobHrefs[job.id]} key={job.id} className="group grid grid-cols-[auto_1fr_auto] items-center gap-4 rounded-2xl border border-[var(--line)] p-5 transition-all hover:-translate-y-1 hover:border-[var(--ink)] hover:shadow-lg">
            ...
            <div>
              {job.urgent && <span className="mb-1.5 mr-2 inline-block rounded-full bg-[var(--coral)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">Urgently hiring</span>}
              {job.unclaimed && <span className="mb-1.5 inline-block rounded-full bg-[var(--yellow)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--ink)]">Unclaimed</span>}
