...
- **US-62:** As an employer, I want the system to auto-generate a must-have checklist from the plain-language description I write, so I don't have to build the checklist by hand.
  - *AC:* Must-have items are extracted only from what the employer explicitly stated in their responsibilities/description — never invented. Included/checked by default since they reflect the employer's own words, but remain individually editable or removable before saving, same as any other AI-touched content under §5.
- **US-63:** As an employer, I want the system to suggest common "preferred" requirements for this type of role, so I don't have to think of every relevant qualification myself.
  - *AC:* Preferred suggestions are shown unchecked by default and visually labeled as AI-generated suggestions, not employer statements. The employer must individually select each one to add it — no bulk "accept all," no pre-checked preferred items. This is the one place in the product where AI content isn't grounded in something the employer already said (§5's general rule) — the guardrail here is procedural instead: every suggestion requires its own explicit, individual opt-in before it becomes part of the listing.
