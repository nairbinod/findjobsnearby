          ...
          <div className="mt-5 grid gap-4">
            {visibleJobs.map((job) => (
              <Link href={jobHref(job)} key={job.id} className="grid gap-5 rounded-2xl border border-[var(--line)] bg-white p-6 transition-all hover:-translate-y-1 hover:border-[var(--ink)] hover:shadow-lg sm:grid-cols-[1fr_auto] sm:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-3"><h3 className="text-xl font-bold">{job.title}</h3><span className="rounded-full bg-[var(--mint)] px-3 py-1 text-[11px] font-bold uppercase">Open</span>{job.urgent && <span className="rounded-full bg-[var(--coral)] px-3 py-1 text-[11px] font-bold uppercase text-white">Urgently hiring</span>}{job.unclaimed && <span className="rounded-full bg-[var(--yellow)] px-3 py-1 text-[11px] font-bold uppercase text-[var(--ink)]">Unclaimed</span>}</div>
