...
export default function AccountPage() {
  ...
  // Bumped on every refresh and used as part of MessageThread's key -- it
  // doesn't refetch on its own otherwise, since its own props never change.
  ...
  async function loadAccount() {
    ...
    setCandidateId(userData.user.id);

    // US-71: landed here straight from confirming an email-only application --
    // welcome them into the account that confirmation just created.
    if (new URLSearchParams(window.location.search).get("applied")) {
      setWelcomeMessage("Your application is confirmed and sent. Welcome to your account.");
    }
  ...
  useEffect(() => {
    // loadAccount is defined at component scope (not inside this effect) so
    // the Refresh button can call it too -- safe to run once on mount either way.
  ...
  return (
    <div className="min-h-screen bg-[var(--cream)]">
      ...
      <main className="mx-auto max-w-[1100px] px-6 pb-20 pt-12 lg:px-10"><p className="mb-4 text-xs font-bold uppercase tracking-[.2em] text-[var(--coral)]">Your workspace</p><h1 className="display text-5xl font-bold leading-[.95] tracking-[-.04em] sm:text-7xl">Keep track<br />of your next move.</h1>{welcomeMessage && <p role="status" className="mt-4 rounded-xl bg-[var(--mint)] px-4 py-3 text-sm font-semibold">{welcomeMessage}</p>}{loading ? <p className="mt-12 text-[var(--muted)]">Loading your workspace...</p> : message ? <div className="mt-12 rounded-2xl border border-[var(--line)] bg-white p-8"><p className="text-lg font-semibold">{message}</p><a href="/applicant/auth" className="mt-5 inline-block rounded-full bg-[var(--coral)] px-6 py-3 text-sm font-bold text-white">Sign in <span aria-hidden="true">→</span></a></div> : <div className="mt-12 grid gap-12 lg:grid-cols-[1.1fr_.9fr]"><section><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-[var(--coral)]">US-50 / US-51</p><h2 className="display mt-3 text-3xl font-bold">My applications</h2></div><div className="flex items-center gap-3"><span className="text-sm text-[var(--muted)]">{applications.length} total</span><button onClick={() => void loadAccount()} className="text-xs font-bold text-[var(--coral)]">Refresh</button></div></div>{applications.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--line)] p-8 text-[var(--muted)]">Your applications will appear here after you apply to a role.</div> : <div className="mt-6 space-y-3">{applications.map((application) => <article key={application.id} className={`rounded-2xl border border-[var(--line)] bg-white p-5 ${application.withdrawn_at ? "opacity-60" : ""}`}><div className="flex items-start justify-between gap-4"><div><h3 className="font-bold">{application.jobs?.title ?? "Job listing"}</h3><p className="mt-1 text-sm text-[var(--muted)]">{application.jobs?.company_name ?? "Local employer"}</p></div><span className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase ${application.withdrawn_at ? "bg-[var(--line)] text-[var(--muted)]" : "bg-[var(--mint)]"}`}>{application.withdrawn_at ? "Withdrawn" : "Applied"}</span></div><div className="mt-5 flex flex-wrap items-center gap-4 border-t border-[var(--line)] pt-4 text-xs text-[var(--muted)]"><span>Applied {new Date(application.created_at).toLocaleDateString()}</span><span>Profile: {application.candidate_profiles?.role_title ?? "Focused profile"}</span><span>Employer view: {application.jobs && viewedByEmployerIds.has(application.jobs.employer_id) ? "Viewed" : "Not yet viewed"}</span>{!application.withdrawn_at && <button onClick={() => void withdrawApplication(application.id)} className="font-bold text-[var(--coral)]">Withdraw</button>}</div>{candidateId && application.jobs && viewedByEmployerIds.has(application.jobs.employer_id) && <MessageThread key={`${application.jobs.employer_id}-${refreshToken}`} employerId={application.jobs.employer_id} candidateId={candidateId} viewerId={candidateId} counterpartLabel="employer" />}</article>)}</div>}</section><section><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-[var(--coral)]">US-11</p><h2 className="display mt-3 text-3xl font-bold">Your profiles</h2></div><span className="text-sm text-[var(--muted)]">{profiles.length}/5</span></div>{profiles.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--line)] p-8 text-[var(--muted)]">Create a profile when you apply for a role.</div> : <div className="mt-6 space-y-3">{profiles.map((profile) => <article key={profile.id} className="rounded-2xl border border-[var(--line)] bg-white p-5"><div className="flex items-center justify-between"><h3 className="font-bold">{profile.role_title}</h3><span className="text-xs font-semibold text-[var(--muted)]">{profile.approved_at ? "Approved" : "Draft"}</span></div><p className="mt-2 text-sm text-[var(--muted)]">Available: {profile.availability || "Not specified"}</p>{formatWindow(profile.available_from, profile.available_until) && <p className="mt-1 text-xs font-semibold text-[var(--coral)]">{formatWindow(profile.available_from, profile.available_until)}</p>}</article>)}</div>}</section></div>}{!loading && !message && referralCode && <div className="mt-12"><ReferralCard referralCode={referralCode} /></div>}</main>
    </div>
  );
}
